...
## Inicie um projeto com o NuvolsCore

Inicialmente, você deve fazer as alterações em um projeto gerado com o NuvolsCore e, assim que finalizar, verifique o resultado das alterações. Se estiver tudo certo, transcreva as alterações para o seu fork. Esse processo é necessário pois, para ver o resultado das alterações, você deve rodar a documentação do projeto localmente e isso só é possível se você tiver um projeto gerado com o NuvolsCore. Para iniciar um projeto com o NuvolsCore, siga o passo a passo do [readme](https://github.com/nuvols//#readme) do projeto.
...
Faça as alterações que deseja no projeto iniciado com o NuvolsCore.
...
Após finalizar suas alterações do projeto iniciado com o Nuvols Core, você deve transcrever elas para o seu fork.
...
Antes de abrir um Pull Request, você precisa garantir que seu fork está atualizado com a branch `desenvolvimento` do projeto NuvolsCore, pois neste meio tempo pode ter ocorrido alguma alteração nessa branch, gerando conflitos com suas alterações. Para que isso não ocorra, basta atualizar seu fork executando o comando `git pull upstream desenvolvimento`.

## Abra uma Pull Request (PR)

Após adicionar as alterações no seu fork e atualizar a branch, você deve abrir um Pull Request para a branch `desenvolvimento` do projeto NuvolsCore. Para isso, basta clicar no botão `Compare & pull request` ou `Open pull request` no seu fork.
> Lembre-se de confirmar se a PR está sendo aberta da branch `desenvolvimento` do seu fork para a branch `desenvolvimento` do projeto NuvolsCore.
