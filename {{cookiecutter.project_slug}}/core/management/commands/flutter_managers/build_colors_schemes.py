...
class ColorsSchemeBuilder:
    def __init__(self, command) -> None:
        self.command = command
        self._snippet_dir = Path(f"{self.command.path_command}/snippets/flutter/")
        self._log_path_file = Path(f"{self.command.core_dir}/color.schemes.dart")
    ...
    def build(self):
        ...
        """
        try:
            _content = Utils.get_snippet(str(Path(f"{self._snippet_dir}/color.schemes.txt")))
