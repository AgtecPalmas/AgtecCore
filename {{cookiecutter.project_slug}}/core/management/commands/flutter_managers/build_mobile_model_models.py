...
class AppsMobileModelBuilder:
    def __init__(self, command, app) -> None:
        ...
        self.flutter_snippet = Path(
            f"{self.command.path_command}/snippets/flutter_mobile_project/layers/model.txt"
        )
    ...
    def build(self):
        try:
            ...
            for model in _models_app:
                ...
                _app_file = Path(
                    f"{self.flutter_dir}/lib/apps/{_model_app_lower}/models/{_model_name_lower}.dart"
                )
                _list_fields_parser = []
    ...
    def _parser_content(self, model):
        _snippet_content = Utils.get_snippet(str(self.flutter_snippet))
