...
class TranslateStringBuilder:
    def __init__(self, command) -> None:
        ...
        self._path_localization_file = os.path.join(
            self.command.core_dir, "localization.dart"
        )
        self._snippet_file = Utils.get_snippet(
            str(Path(f"{self.snippet_dir}/localization.txt"))
        )
    ...
    def build(self):
        try:
            ...
            with open(
                self._path_localization_file, "w", encoding="utf-8"
            ) as localizations:
            ...
            if not Utils.check_file(self._pt_br_file):
                _snippet = Utils.get_snippet(
                    str(Path(f"{self.snippet_dir}/pt_language.txt"))
                )
            ...
            if not Utils.check_file(self._en_us_file):
                _snippet = Utils.get_snippet(
                    str(Path(f"{self.snippet_dir}/en_language.txt"))
                )
                with open(self._en_us_file, "w", encoding="utf-8") as en_file:
                    en_file.write(_snippet)
        except Exception as e:
            Utils.show_message(
                f"Erro ao executar o build de TranslateStringBuilder: {e}"
            )
            return
