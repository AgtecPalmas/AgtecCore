    ...
    def manage_router_urls(self):
        try:
            ...
            if Utils.check_file(self.path_api_urls) is False:
                with open(self.path_api_urls, "w", encoding="utf-8") as api_url_file:
                    api_url_file.write(content)
                Utils.show_message("URLs API criados com sucesso")
                return

            if Utils.check_content(self.path_api_urls, f"{self.model}ViewAPI"):
                Utils.show_message(
            ...
            with open(self.path_api_urls, "r", encoding="utf-8") as api_url_file:
