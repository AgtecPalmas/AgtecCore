...
class AppsMobileDataBuilder:
    def __init__(self, command, app) -> None:
        ...
        self.flutter_snippet = Path(
            f"{self.command.path_command}/snippets/flutter_mobile_project/layers/data.txt"
        )
    ...
    def build(self):
        try:
            ...
            for model in _models_app:
                ...
                _app_file = Path(
                    f"{self.flutter_dir}/lib/apps/{_model_app_lower}/data/{_model_name_lower}.dart"
                )
                _list_fields_parser = []
