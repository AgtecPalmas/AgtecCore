...
Aqui será feita a cópia pura, sem necessidade de mapear as classes
do projeto Django
...
class AppsMobileRouterInjectRootRouteBuilder:
        ...
        try:
            ...
            for app in FLUTTER_APPS_WEB:
                ...
                for model in _app.models:
                    if len(model) < 2:
                        continue
                    # Adicionando os routes no projeto
                    _list_imports += f"import '../apps/{app.lower()}/routes/{model[2]}.dart';\n"
                    _list_routers += f"...{model[2]}Routes,\n"

            # Adicionando os routes no projeto
            _routers_file = Path(f"{self.flutter_dir}/lib/core/app.routes.dart")
