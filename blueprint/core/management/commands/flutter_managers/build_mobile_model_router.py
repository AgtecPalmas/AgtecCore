...
class AppsMobileRouterBuilder:
    def __init__(self, command, app) -> None:
        self.command = command
        self.app = app
        self.flutter_dir = self.command.flutter_dir
        self.flutter_snippet = Path(
            f"{self.command.path_command}/snippets/flutter_mobile_project/layers/router.txt"
        )
    ...
    def build(self):
        ...
        try:
            ...
            for model in self._models_app:
                ...
                _app_file = Path(f"{self.flutter_dir}/lib/apps/{_model_app_lower}/routes/{_model_name_lower}.dart")

                # Parseando o snippet
                _content = Utils.get_snippet(self.flutter_snippet)
